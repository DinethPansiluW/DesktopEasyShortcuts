@echo off
title Shutdown Countdown Generator
...
:: Create Shutdown.bat with the countdown script
(
  echo @echo off
  echo title Shutdown Countdown
  echo color 0A
  echo setlocal enabledelayedexpansion
  echo set "Time=%Time%"
  echo cls
  echo powershell -Command "Write-Host 'Countdown Start to Shutdown' -ForegroundColor Cyan"
  echo echo.
  echo for /l %%%%i in ^(!Time!,-1,1^) do ^(
  echo    echo Shutting down in %%%%i seconds...
  echo echo.
