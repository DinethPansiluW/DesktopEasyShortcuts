...
:MENU_LANGUAGE
cls
title Installer x64 For Package 1 By GuCATs
echo Installer x64 For Package 1
echo.
...
:: Install all .Appx and .AppxBundle files in the Microsoft_Store folder
...
:: Install all .Appx and .AppxBundle files in the AppInstaller folder

echo.
echo Installing AppInstaller...
for %%f in (Microsoft_Store\Microsoft.UI.Xaml.2.8_*.Appx Microsoft_Store\Microsoft.VCLibs.140.00.UWPDesktop_*.Appx) do (
    echo Installing %%f...
    powershell.exe -Command "Add-AppxPackage -Path %%f"
)
...
:: Install all .Appx and .AppxBundle files in the Microsoft_Store and AppInstaller folder
...
:: 安装 Microsoft_Store 文件夹下所有的 .Appx 和 AppxBundle 文件
...
:: 安装 AppInstaller 文件夹下所有的 .Appx 和 AppxBundle 文件
...
:: 安装 Microsoft_Store 和 AppInstaller 文件夹下所有的 .Appx 和 AppxBundle 文件
